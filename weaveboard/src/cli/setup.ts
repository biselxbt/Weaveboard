 ...
 * so the Weaveboard MCP server is available in all projects.
 */
...
function getMcpEntry() {
  if (process.platform === 'win32') {
    return {
      command: 'cmd',
      args: ['/c', 'npx', '-y', 'weaveboard@latest', 'mcp'],
    };
  }
  return {
    command: 'npx',
...
/**
 * Merge weaveboard entry into an existing MCP config JSON object.
...
function mergeMcpConfig(existing: any): any {
  ...
  existing.mcpServers.weaveboard = getMcpEntry();
  return existing;
}
...
async function setupClaudeCode(result: SetupResult): Promise<void> {
  ...
  console.log('');
  console.log('  Claude Code detected. Run this command to add Weaveboard MCP:');
  console.log('');
  console.log('    claude mcp add weaveboard -- npx -y weaveboard mcp');
  console.log('');
  result.configured.push('Claude Code (MCP manual step printed)');
}

/**
 * Install Weaveboard skills to ~/.claude/skills/ for Claude Code.
 */
...
/**
 * Install Weaveboard hooks to ~/.claude/settings.json for Claude Code.
...
async function installClaudeCodeHooks(result: SetupResult): Promise<void> {
  ...
  // Source hooks bundled within the weaveboard package (hooks/claude/)
  const pluginHooksPath = path.join(__dirname, '..', '..', 'hooks', 'claude');

  // Copy unified hook script to ~/.claude/hooks/weaveboard/
  const destHooksDir = path.join(claudeDir, 'hooks', 'weaveboard');

  try {
    await fs.mkdir(destHooksDir, { recursive: true });

    const src = path.join(pluginHooksPath, 'weaveboard-hook.cjs');
    const dest = path.join(destHooksDir, 'weaveboard-hook.cjs');
    try {
    ...
    const hookCmd = `node "${path.join(destHooksDir, 'weaveboard-hook.cjs').replace(/\\/g, '/')}"`;
    ...
    const hasPreToolHook = existing.hooks.PreToolUse.some(
      (h: any) => h.hooks?.some((hh: any) => hh.command?.includes('weaveboard'))
    );
    if (!hasPreToolHook) {
      existing.hooks.PreToolUse.push({
        ...
        hooks: [{
          ...
          command: hookCmd,
          timeout: 8000,
          statusMessage: 'Enriching with Weaveboard graph context...',
        }],
      });
    }
...
async function setupOpenCode(result: SetupResult): Promise<void> {
  ...
  try {
    ...
    config.mcp.weaveboard = getMcpEntry();
...
const SKILL_NAMES = ['weaveboard-exploring', 'weaveboard-debugging', 'weaveboard-impact-analysis', 'weaveboard-refactoring', 'weaveboard-guide', 'weaveboard-cli'];

/**
 * Install Weaveboard skills to a target directory.
 * Each skill is installed as {targetDir}/weaveboard-{skillName}/SKILL.md
...
/**
 * Install global Cursor skills to ~/.cursor/skills/weaveboard/
 */
...
/**
 * Install global OpenCode skills to ~/.config/opencode/skill/weaveboard/
 */
...
export const setupCommand = async () => {
  ...
  console.log('    2. Run: weaveboard analyze');
