#!/bin/bash
# Weaveboard PreToolUse hook for Claude Code
...
# Check if we're in a Weaveboard-indexed repo
dir="${CWD:-$PWD}"
found=false
...
# Run weaveboard augment — must be fast (<500ms target)
RESULT=$(cd "$CWD" && npx -y weaveboard augment "$PATTERN" 2>/dev/null)
