    ...
    const handleViewAllProcesses = useCallback(async () => {
        ...
        try {
            ...
            const allStepsQuery = `
                ...
                WHERE p.id IN [${allProcessIds.map(id => `'${escapeCypherString(id)}'`).join(',')}]
            ...
            if (stepIds.length > 0) {
                ...
                const edgesQuery = `
                    ...
                    WHERE from.id IN [${stepIds.map(id => `'${escapeCypherString(id)}'`).join(',')}]
                      AND to.id IN [${stepIds.map(id => `'${escapeCypherString(id)}'`).join(',')}]
