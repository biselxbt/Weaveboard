#!/usr/bin/env node
/**
 * Weaveboard Claude Code Plugin Hook
 *
 * PreToolUse handler — intercepts Grep/Glob/Bash searches
 * and augments with graph context from the Weaveboard index.
 *
...
/**
 * Check if a directory (or ancestor) has a .weaveboard index.
 */
function findWeaveboardIndex(startDir) {
  let dir = startDir || process.cwd();
  for (let i = 0; i < 5; i++) {
    if (fs.existsSync(path.join(dir, '.weaveboard'))) {
      return true;
    }
...
function main() {
  try {
    ...
    if (!findWeaveboardIndex(cwd)) return;
    ...
    // Try direct weaveboard binary first (faster if globally installed)
    try {
