...
import { WEAVEBOARD_TOOLS } from './tools.js';
...
function getNextStepHint(toolName: string, args: Record<string, any> | undefined): string {
  ...
  switch (toolName) {
    case 'list_repos':
      return `\n\n---\n**Next:** READ weaveboard://repo/{name}/context for any repo above to get its overview and check staleness.`;
    ...
    case 'context':
      return `\n\n---\n**Next:** If planning changes, use impact({target: "${args?.name || '<name>'}", direction: "upstream"${repoParam}}) to check blast radius. To see execution flows, READ weaveboard://repo/${repoPath}/processes.`;

    case 'impact':
      return `\n\n---\n**Next:** Review d=1 items first (WILL BREAK). To check affected execution flows, READ weaveboard://repo/${repoPath}/processes.`;

    case 'detect_changes':
      return `\n\n---\n**Next:** Review affected processes. Use context() on high-risk changed symbols. READ weaveboard://repo/${repoPath}/process/{name} for full execution traces.`;

    case 'rename':
      return `\n\n---\n**Next:** Run detect_changes(${repoParam ? `{repo: "${repo}"}` : ''}) to verify no unexpected side effects from the rename.`;

    case 'cypher':
      return `\n\n---\n**Next:** To explore a result symbol, use context({name: "<name>"${repoParam}}). For schema reference, READ weaveboard://repo/${repoPath}/schema.`;
    ...
    case 'overview':
      return `\n\n---\n**Next:** To drill into an area, READ weaveboard://repo/${repoPath}/cluster/{name}. To see execution flows, READ weaveboard://repo/${repoPath}/processes.`;
...
export function createMCPServer(backend: LocalBackend): Server {
  ...
  server.setRequestHandler(ListToolsRequestSchema, async () => ({
    tools: WEAVEBOARD_TOOLS.map((tool) => ({
      name: tool.name,
...
Follow these steps:
1. READ \`weaveboard://repo/${repo || '{name}'}/context\` for codebase stats
2. READ \`weaveboard://repo/${repo || '{name}'}/clusters\` to see all functional areas
3. READ \`weaveboard://repo/${repo || '{name}'}/processes\` to see all execution flows
4. For the top 5 most important processes, READ \`weaveboard://repo/${repo || '{name}'}/process/{name}\` for step-by-step traces
