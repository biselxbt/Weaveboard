...
const VALID_NODE_LABELS = new Set([
  'File', 'Folder', 'Function', 'Class', 'Method', 'Interface', 'Variable', 'Import', 'Type', 'Process', 'Community'
]);
  ...
  const searchTool = tool(
    async ({ query, limit, groupByProcess }: { query: string; limit?: number; groupByProcess?: boolean }) => {
      ...
      for (let i = 0; i < Math.min(searchResults.length, k); i++) {
        ...
        if (nodeId) {
          try {
            const nodeLabel = validateLabel(nodeId.split(':')[0]);
            const escapedNodeId = escapeCypherString(nodeId);
            const connectionsQuery = `
              MATCH (n:${nodeLabel} {id: '${escapedNodeId}'})
...
MATCH (n:Function {id: emb.nodeId}) RETURN n`,
  ...
  const exploreTool = tool(
    async ({ target, type }: { target: string; type?: 'symbol' | 'cluster' | 'process' | null }) => {
      const safeTarget = escapeCypherString(target);
      ...
      if (resolvedType === 'process') {
        ...
        const stepsQuery = `
          MATCH (s)-[r:CodeRelation {type: 'STEP_IN_PROCESS'}]->(p:Process {id: '${escapeCypherString(pid)}'})
          RETURN s.name AS name, s.filePath AS filePath, r.step AS step
          ORDER BY r.step
        `;
        const clustersQuery = `
          MATCH (c:Community)<-[:CodeRelation {type: 'MEMBER_OF'}]-(s)
          MATCH (s)-[:CodeRelation {type: 'STEP_IN_PROCESS'}]->(p:Process {id: '${escapeCypherString(pid)}'})
      ...
      if (resolvedType === 'cluster') {
        ...
        const membersQuery = `
          MATCH (c:Community {id: '${escapeCypherString(cid)}'})<-[:CodeRelation {type: 'MEMBER_OF'}]-(m)
          RETURN m.name AS name, m.filePath AS filePath, label(m) AS nodeType
          LIMIT 50
        `;
        const processesQuery = `
          MATCH (c:Community {id: '${escapeCypherString(cid)}'})<-[:CodeRelation {type: 'MEMBER_OF'}]-(s)
      ...
      if (resolvedType === 'symbol') {
        ...
        const clusterQuery = `
          MATCH (n:${validateLabel(nodeType)} {id: '${escapeCypherString(String(nodeId))}'})
          MATCH (n)-[:CodeRelation {type: 'MEMBER_OF'}]->(c:Community)
  ...
  const impactTool = tool(
    async ({ target, direction, maxDepth, relationTypes, includeTests, minConfidence }: { 
      ...
      const d1Query = direction === 'upstream'
        ? isFileTarget
          ? `
            ...
            WHERE callee.filePath = '${escapeCypherString(targetFilePath || target)}'
        ...
        : isFileTarget
          ? `
            ...
            WHERE caller.filePath = '${escapeCypherString(targetFilePath || target)}'
              AND r.type IN [${relTypeFilter}]
      ...
      const idList = trimmedIds.map(id => `'${escapeCypherString(id)}'`).join(', ');
      ...
      if (trimmedIds.length > 0) {
        ...
        const clusterQuery = `
          ...
          LIMIT 20
        `;
        const directIdList = depth1.map(n => `'${escapeCypherString(n.id)}'`).join(', ');
