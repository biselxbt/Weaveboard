 ...
 * Repository Manager
 * 
 * Manages Weaveboard index storage in .weaveboard/ at repo root.
 * Also maintains a global registry at ~/.weaveboard/registry.json
...
/**
 * Shape of an entry in the global registry (~/.weaveboard/registry.json)
 */
...
const WEAVEBOARD_DIR = '.weaveboard';

// ─── Local Storage Helpers ─────────────────────────────────────────────

/**
 * Get the .weaveboard storage path for a repository
 */
export const getStoragePath = (repoPath: string): string => {
  return path.join(path.resolve(repoPath), WEAVEBOARD_DIR);
};
...
/**
 * Check if a path has a Weaveboard index
 */
...
/**
 * Find .weaveboard by walking up from a starting path
 */
...
/**
 * Add .weaveboard to .gitignore if not already present
 */
export const addToGitignore = async (repoPath: string): Promise<void> => {
  ...
  try {
    ...
    if (content.includes(WEAVEBOARD_DIR)) return;
    
    const newContent = content.endsWith('\n') 
      ? `${content}${WEAVEBOARD_DIR}\n`
      : `${content}\n${WEAVEBOARD_DIR}\n`;
    await fs.writeFile(gitignorePath, newContent, 'utf-8');
  } catch {
    // .gitignore doesn't exist, create it
    await fs.writeFile(gitignorePath, `${WEAVEBOARD_DIR}\n`, 'utf-8');
  }
};

// ─── Global Registry (~/.weaveboard/registry.json) ───────────────────────

/**
 * Get the path to the global Weaveboard directory
 */
export const getGlobalDir = (): string => {
  return path.join(os.homedir(), '.weaveboard');
};
 ...
 * Called after `weaveboard analyze` completes.
 */
 ...
 * Optionally validates that each entry's .weaveboard/ still exists.
 */
export const listRegisteredRepos = async (opts?: { validate?: boolean }): Promise<RegistryEntry[]> => {
  const entries = await readRegistry();
  if (!opts?.validate) return entries;

  // Validate each entry still has a .weaveboard/ directory
...
// ─── Global CLI Config (~/.weaveboard/config.json) ─────────────────────────
...
/**
 * Load CLI config from ~/.weaveboard/config.json
 */
...
/**
 * Save CLI config to ~/.weaveboard/config.json
 */
