 ...
 * 
 * Usage:
 *   weaveboard eval-server                    # default port 4848
 *   weaveboard eval-server --port 4848        # explicit port
 *   weaveboard eval-server --idle-timeout 300 # auto-shutdown after 300s idle
 * 
 * API:
...
function formatContextResult(result: any): string {
  ...
  if (result.status === 'ambiguous') {
    ...
    lines.push(`\nRe-run: weaveboard-context "${result.candidates?.[0]?.name}" "<file_path>"`);
    return lines.join('\n');
  }
...
function getNextStepHint(toolName: string): string {
  switch (toolName) {
    case 'query':
      return '\n---\nNext: Pick a symbol above and run weaveboard-context "<name>" to see all its callers, callees, and execution flows.';

    case 'context':
      return '\n---\nNext: To check what breaks if you change this, run weaveboard-impact "<name>" upstream';

    case 'impact':
      return '\n---\nNext: Review d=1 items first (WILL BREAK). Read the source with cat to understand the code, then make your fix.';

    case 'cypher':
      return '\n---\nNext: To explore a result symbol in depth, run weaveboard-context "<name>"';

    case 'detect_changes':
      return '\n---\nNext: Run weaveboard-context "<symbol>" on high-risk changed symbols to check their callers.';
...
export async function evalServerCommand(options?: EvalServerOptions): Promise<void> {
  ...
  if (!ok) {
    console.error('Weaveboard eval-server: No indexed repositories found. Run: weaveboard analyze');
    process.exit(1);
  }

  const repos = await backend.listRepos();
  console.error(`Weaveboard eval-server: ${repos.length} repo(s) loaded: ${repos.map(r => r.name).join(', ')}`);
  ...
  function resetIdleTimer() {
    ...
    idleTimer = setTimeout(async () => {
      console.error('Weaveboard eval-server: Idle timeout reached, shutting down');
  ...
  server.listen(port, '127.0.0.1', () => {
    console.error(`Weaveboard eval-server: listening on http://127.0.0.1:${port}`);
    ...
    try {
      process.stdout.write(`WEAVEBOARD_EVAL_SERVER_READY:${port}\n`);
    } catch {
  ...
  const shutdown = async () => {
    console.error('Weaveboard eval-server: shutting down...');
    await backend.disconnect();
    server.close();
    process.exit(0);
  };
