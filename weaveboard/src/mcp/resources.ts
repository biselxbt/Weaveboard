 ...
 * All resources use repo-scoped URIs: weaveboard://repo/{name}/context
 */
...
export function getResourceDefinitions(): ResourceDefinition[] {
  return [
    ...
    {
      uri: 'weaveboard://setup',
      name: 'Weaveboard Setup Content',
...
export function getResourceTemplates(): ResourceTemplate[] {
  return [
    {
      uriTemplate: 'weaveboard://repo/{name}/context',
      name: 'Repo Overview',
      description: 'Codebase stats, staleness check, and available tools',
      mimeType: 'text/yaml',
    },
    {
      uriTemplate: 'weaveboard://repo/{name}/clusters',
      name: 'Repo Modules',
      description: 'All functional areas (Leiden clusters)',
      mimeType: 'text/yaml',
    },
    {
      uriTemplate: 'weaveboard://repo/{name}/processes',
      name: 'Repo Processes',
      description: 'All execution flows',
      mimeType: 'text/yaml',
    },
    {
      uriTemplate: 'weaveboard://repo/{name}/schema',
      name: 'Graph Schema',
      description: 'Node/edge schema for Cypher queries',
      mimeType: 'text/yaml',
    },
    {
      uriTemplate: 'weaveboard://repo/{name}/cluster/{clusterName}',
      name: 'Module Detail',
      description: 'Deep dive into a specific functional area',
      mimeType: 'text/yaml',
    },
    {
      uriTemplate: 'weaveboard://repo/{name}/process/{processName}',
      name: 'Process Trace',
...
function parseUri(uri: string): { repoName?: string; resourceType: string; param?: string } {
  if (uri === 'weaveboard://repos') return { resourceType: 'repos' };
  if (uri === 'weaveboard://setup') return { resourceType: 'setup' };

  // Repo-scoped: weaveboard://repo/{name}/context
  const repoMatch = uri.match(/^weaveboard:\/\/repo\/([^/]+)\/(.+)$/);
  if (repoMatch) {
...
async function getReposResource(backend: LocalBackend): Promise<string> {
  ...
  if (repos.length === 0) {
    return 'repos: []\n# No repositories indexed. Run: weaveboard analyze';
  }
  ...
  if (repos.length > 1) {
    ...
    lines.push(`# weaveboard_search({query: "auth", repo: "${repos[0].name}"})`);
  }
...
async function getContextResource(backend: LocalBackend, repoName?: string): Promise<string> {
  ...
  if (!context) {
    return 'error: No codebase loaded. Run: weaveboard analyze';
  }
  ...
  lines.push('');
  lines.push('re_index: Run `npx weaveboard analyze` in terminal if data is stale');
  lines.push('');
  lines.push('resources_available:');
  lines.push('  - weaveboard://repos: All indexed repositories');
  lines.push(`  - weaveboard://repo/${context.projectName}/clusters: All functional areas`);
  lines.push(`  - weaveboard://repo/${context.projectName}/processes: All execution flows`);
  lines.push(`  - weaveboard://repo/${context.projectName}/cluster/{name}: Module details`);
  lines.push(`  - weaveboard://repo/${context.projectName}/process/{name}: Process trace`);
...
async function getClustersResource(backend: LocalBackend, repoName?: string): Promise<string> {
  try {
    ...
    if (!result.clusters || result.clusters.length === 0) {
      return 'modules: []\n# No functional areas detected. Run: weaveboard analyze';
    }
    ...
    if (result.clusters.length > displayLimit) {
      lines.push(`\n# Showing top ${displayLimit} of ${result.clusters.length} modules. Use weaveboard_query for deeper search.`);
    }
...
async function getProcessesResource(backend: LocalBackend, repoName?: string): Promise<string> {
  try {
    ...
    if (!result.processes || result.processes.length === 0) {
      return 'processes: []\n# No processes detected. Run: weaveboard analyze';
    }
    ...
    if (result.processes.length > displayLimit) {
      lines.push(`\n# Showing top ${displayLimit} of ${result.processes.length} processes. Use weaveboard_query for deeper search.`);
    }
...
function getSchemaResource(): string {
  return `# Weaveboard Graph Schema
 ...
 * Useful for `weaveboard setup` onboarding or dynamic content injection.
 */
async function getSetupResource(backend: LocalBackend): Promise<string> {
  const repos = await backend.listRepos();

  if (repos.length === 0) {
    return '# Weaveboard\n\nNo repositories indexed. Run: `npx weaveboard analyze` in a repository.';
  }
  ...
  for (const repo of repos) {
    ...
    const lines = [
      `# Weaveboard MCP — ${repo.name}`,
      '',
      `This project is indexed by Weaveboard as **${repo.name}** (${stats.nodes || 0} symbols, ${stats.edges || 0} relationships, ${stats.processes || 0} execution flows).`,
      '',
      '## Tools',
      '',
      ...
      '## Resources',
      '',
      `- \`weaveboard://repo/${repo.name}/context\` — Stats, staleness check`,
      `- \`weaveboard://repo/${repo.name}/clusters\` — All functional areas`,
      `- \`weaveboard://repo/${repo.name}/processes\` — All execution flows`,
      `- \`weaveboard://repo/${repo.name}/schema\` — Graph schema for Cypher`,
    ];
