...
export const RightPanel = () => {
  ...
  const handleSendMessage = async () => {
    ...
    // Prevent sending if agent is not ready and not initializing
    if (!isAgentReady && !isAgentInitializing) {
      setAgentError('Please configure an AI provider in Settings first');
      return;
    }
  ...
  return (
    <aside className="w-full sm:w-[40%] sm:min-w-[320px] sm:max-w-[600px] flex flex-col glass border-l border-border-subtle animate-slide-in relative z-30 flex-shrink-0 fixed sm:relative inset-0 sm:inset-auto pt-14 sm:pt-0 pb-20 sm:pb-0 overflow-hidden">
      {/* Header with Tabs */}
      <div className="flex items-center justify-between px-3 sm:px-4 py-2 border-b border-border-subtle" role="tablist">
        <div className="flex items-center gap-1" role="presentation">
          ...
          <button
            ...
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-sm font-medium transition-colors ${activeTab === 'chat'
              ? 'bg-white text-black'
              : 'text-text-muted hover:text-white hover:bg-white/10'
              }`}
          >
            <Sparkles className="w-3.5 h-3.5" />
            <span className="hidden sm:inline">Weaveboard AI</span>
          ...
          <button
            ...
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-sm font-medium transition-colors ${activeTab === 'processes'
              ? 'bg-white text-black'
              : 'text-text-muted hover:text-white hover:bg-white/10'
              }`}
          >
            ...
            <span>Processes</span>
            <span className="text-[10px] px-1.5 py-0.5 bg-purple-500/30 text-purple-200 rounded-full font-semibold">
              BETA
            </span>
          </button>
        </div>
      ...
      {activeTab === 'chat' && (
        <div className="flex-1 flex flex-col overflow-hidden">
          {/* Status bar */}
          <div className="flex items-center gap-2.5 px-4 py-3 glass border-b border-border-subtle">
            <div className="ml-auto flex items-center gap-2">
              {!isAgentReady && (
                <span className="text-[11px] px-2 py-1 rounded-full bg-white/10 text-white border border-white/20">
                  Configure AI
                </span>
              )}
              {isAgentInitializing && (
                <span className="text-[11px] px-2 py-1 rounded-full glass flex items-center gap-1 text-text-muted">
          ...
          {/* Status / errors */}
          {agentError && (
            <div className="px-4 py-3 bg-red-500/10 border-b border-red-500/30 text-red-100 text-sm flex items-center gap-2">
          ...
          <div className="flex-1 overflow-y-auto p-3 sm:p-4 scrollbar-thin">
            {chatMessages.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-full text-center px-4">
                <div className="w-14 h-14 mb-4 flex items-center justify-center glass rounded-xl border border-white/20 text-2xl">
                  🧠
                </div>
                    ...
                    {message.role === 'assistant' && (
                      <div>
                        <div className="flex items-center gap-2 mb-3">
                          ...
                          <span className="text-xs font-medium text-text-muted uppercase tracking-wide">Weaveboard AI</span>
          ...
          {/* Input */}
          <div className="p-2 sm:p-3 bg-surface border-t border-border-subtle">
            <div className="flex items-end gap-2 px-3 py-2 bg-elevated border border-border-subtle rounded-xl transition-all focus-within:border-accent focus-within:ring-2 focus-within:ring-accent/20">
              ...
              {isChatLoading ? (
                ...
                <button
                  ...
                  disabled={!chatInput.trim() || isAgentInitializing || !isAgentReady}
                  className="w-9 h-9 flex items-center justify-center bg-white/10 border border-white/20 rounded-md text-white transition-all hover:bg-white/20 disabled:opacity-50 disabled:cursor-not-allowed"
                  title={!isAgentReady ? 'Configure AI provider first' : 'Send message'}
                >
              ...
              <div className="mt-2 text-xs text-amber-200 flex items-center gap-2">
                ...
                <span>
                  {isProviderConfigured()
                    ...
                    : 'Configure an LLM provider OR hold $WEAVE tokens for premium AI (coming this week!)'}
                </span>
              </div>
            )}
          </div>
