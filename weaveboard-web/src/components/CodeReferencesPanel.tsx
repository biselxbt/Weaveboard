...
export const CodeReferencesPanel = ({ onFocusNode }: CodeReferencesPanelProps) => {
  ...
  const [panelWidth, setPanelWidth] = useState<number>(() => {
    try {
      const saved = window.localStorage.getItem('weaveboard.codePanelWidth');
  ...
  useEffect(() => {
    try {
      window.localStorage.setItem('weaveboard.codePanelWidth', String(panelWidth));
    } catch {
      // ignore
    }
  }, [panelWidth]);
        ...
        {showSelectedViewer && (
          <div className={`${showCitations ? 'h-[42%]' : 'flex-1'} min-h-0 flex flex-col`}>
            <div className="px-3 py-2 bg-gradient-to-r from-amber-500/8 to-orange-500/5 border-b border-amber-500/20 flex items-center gap-2">
              ...
              {/* Mobile: prominent clear button */}
              <button
                onClick={() => setSelectedNode(null)}
                className="sm:hidden flex items-center gap-1 px-2 py-1 bg-amber-500/20 hover:bg-amber-500/40 text-amber-400 rounded transition-colors text-xs font-medium"
                title="Clear selection"
              >
                <X className="w-3.5 h-3.5" />
                Clear
              </button>
              {/* Desktop: smaller clear button */}
              <button
                onClick={() => setSelectedNode(null)}
                className="hidden sm:flex p-1 text-text-muted hover:text-amber-400 hover:bg-amber-500/10 rounded transition-colors"
                title="Clear selection"
              >
                <X className="w-4 h-4" />
              </button>
