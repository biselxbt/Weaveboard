...
const VALID_NODE_LABELS = new Set(['File', 'Folder', 'Function', 'Class', 'Method', 'Interface', 'Variable', 'Import', 'Type', 'Process', 'Community']);
  ...
  try {
    ...
    for (const line of relLines) {
      try {
        ...
        const insertQuery = `
          MATCH (a:${escapeLabel(fromLabel)} {id: '${escapeCypherString(fromId)}'}),
                (b:${escapeLabel(toLabel)} {id: '${escapeCypherString(toId)}'})
          CREATE (a)-[:${REL_TABLE_NAME} {type: '${escapeCypherString(relType)}', confidence: ${confidence}, reason: '${escapeCypherString(reason)}', step: ${step}}]->(b)
        `;
