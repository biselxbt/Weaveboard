    ...
    <div>
      <button
        ...
        className={`
          ...
          hover:bg-white/10 transition-colors rounded relative
          ${isSelected ? 'bg-white/15 text-white border-l-2 border-white' : 'text-text-secondary hover:text-white border-l-2 border-transparent'}
          ${matchesSearch ? 'bg-white/10' : ''}
        `}
...
export const FileTreePanel = ({ onFocusNode }: FileTreePanelProps) => {
  const { graph, visibleLabels, toggleLabelVisibility, visibleEdgeTypes, toggleEdgeVisibility, selectedNode, setSelectedNode, openCodePanel, depthFilter, setDepthFilter, isFileTreeOpen, setFileTreeOpen, isRightPanelOpen } = useAppState();
  ...
  // Determine if panel should be visible (desktop always, mobile controlled by isFileTreeOpen)
  const isVisible = !isCollapsed && (isFileTreeOpen || isCollapsed === false);
  ...
  if (isCollapsed) {
    return (
      <div className="h-full w-12 glass flex flex-col items-center py-3 gap-2 border-r border-border-subtle">
        <button
        ...
        <button
          ...
          className={`p-2 rounded-full transition-colors ${activeTab === 'files' ? 'text-black bg-white' : 'text-text-secondary hover:text-white hover:bg-white/10'}`}
          title="File Explorer"
        >
        ...
        <button
          ...
          className={`p-2 rounded-full transition-colors ${activeTab === 'filters' ? 'text-black bg-white' : 'text-text-secondary hover:text-white hover:bg-white/10'}`}
          title="Filters"
        >
  ...
  return (
    <div className="h-full w-64 glass border-r border-border-subtle flex flex-col animate-slide-in relative">
      {/* Mobile close button */}
      <button
        onClick={() => setFileTreeOpen(false)}
        className="absolute top-2 right-2 p-1.5 sm:hidden text-text-muted hover:text-white hover:bg-white/10 rounded-full transition-colors z-10"
        title="Close Panel"
      >
        <X className="w-4 h-4" />
      </button>
      ...
      <div className="flex items-center justify-between px-3 py-2 border-b border-border-subtle">
        <div className="flex items-center gap-1">
          <button
            ...
            className={`px-3 py-1.5 text-xs rounded-full transition-colors ${activeTab === 'files'
          ...
          <button
            ...
            className={`px-3 py-1.5 text-xs rounded-full transition-colors ${activeTab === 'filters'
              ? 'bg-white text-black font-medium'
              : 'text-text-secondary hover:text-white hover:bg-white/10'
              }`}
          >
          ...
          <div className="px-3 py-2 border-b border-border-subtle">
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-8 pr-3 py-1.5 glass border border-border-subtle rounded-full text-xs text-white placeholder:text-text-muted focus:outline-none focus:border-white/50"
              />
            </div>
          </div>
