 ...
 * 
 * Registers the Weaveboard PreToolUse hook in ~/.claude/hooks.json
...
/**
 * Get the absolute path to the weaveboard-hook.js file.
 * Works for both local dev and npm-installed packages.
 */
function getHookScriptPath(): string {
  // From dist/cli/claude-hooks.js → hooks/claude/weaveboard-hook.js
  const packageRoot = path.resolve(__dirname, '..', '..');
  return path.join(packageRoot, 'hooks', 'claude', 'weaveboard-hook.cjs');
}

/**
 * Register (or verify) the Weaveboard hook in Claude Code's global hooks.json.
 * 
 * - Creates ~/.claude/ and hooks.json if they don't exist
 * - Preserves existing hooks from other tools
 * - Skips if Weaveboard hook is already registered
 * 
...
export async function registerClaudeHook(): Promise<{ registered: boolean; message: string }> {
  ...
  // Check if Weaveboard hook is already registered
  const existingEntry = hooksConfig.hooks.PreToolUse.find((entry: any) => {
    ...
    return entry.hooks.some((h: any) => 
      h.command && (
        h.command.includes('weaveboard-hook') || 
        h.command.includes('weaveboard augment')
      )
    );
  });
