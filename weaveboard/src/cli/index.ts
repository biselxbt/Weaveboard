...
program
  .name('weaveboard')
  .description('Weaveboard local CLI and MCP server')
  .version(pkg.version);
...
program
  .command('clean')
  .description('Delete Weaveboard index for current repo')
...
program
  ...
  .option('--api-key <key>', 'LLM API key (saved to ~/.weaveboard/config.json)')
