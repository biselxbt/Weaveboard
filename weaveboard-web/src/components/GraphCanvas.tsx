...
export const GraphCanvas = forwardRef<GraphCanvasHandle>((_, ref) => {
  ...
  return (
    <div className="relative w-full h-full bg-void overflow-hidden">
      ...
      {(!graph || graph.nodes.length === 0) && (
        <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
          <div className="text-center max-w-sm px-4">
            ...
            <h3 className="text-lg font-semibold text-white/60 mb-2">No Graph Loaded</h3>
            <p className="text-sm text-text-muted">
              Upload a codebase to visualize its knowledge graph
            </p>
          </div>
        </div>
      )}
      ...
      {/* AI Highlights toggle - Top Right (desktop) / Top Left (mobile) */}
      <div className="absolute top-4 sm:right-4 right-4 left-4 sm:left-auto z-20">
        <button
          onClick={() => {
