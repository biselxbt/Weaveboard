 ...
 * Retries on lock errors (e.g., when `weaveboard analyze` is running).
 */
export const initKuzu = async (repoId: string, dbPath: string): Promise<void> => {
  ...
  try {
    ...
    throw new Error(`KuzuDB not found at ${dbPath}. Run: weaveboard analyze`);
  }
  ...
  // avoids lock conflicts when `weaveboard analyze` is writing.
